...
public class CheckService {
    ...
    public int getLeaveDayNumber(Check check,String leaveType){
        List<Leave> leaves = leaveDao.getLeavesByEmIdAndMonth(check.getEmployeeID(), check.getMonth(), leaveType);
        return calcLeaveDays(leaves, check.getDate());
    }

    private int calcLeaveDays(List<Leave> leaves, String month) {
        int days = 0;
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM");
        YearMonth yearMonth = YearMonth.parse(month, formatter);

        for (Leave leave : leaves) {
            LocalDate startDate = leave.getBeginTime().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            LocalDate endDate = leave.getEndTime().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

            while (!startDate.isAfter(endDate)) {
                if (YearMonth.from(startDate).equals(yearMonth) &&
    ...
    public int getAllLeaveDay(Check check){
        List<Leave> leaves = leaveDao.getLeavesByEmIdAndMonth(check.getEmployeeID(), check.getMonth(), null);
        return calcLeaveDays(leaves, check.getDate());
    }
}
