          ...
          <div style="margin-left: 15px; margin-top: 20px;">
            ...
            <div style="margin-top: 10px;" id="check">
              <el-table :data="checkList" border fit highlight-current-row style="width: 100%">
                ...
                <el-table-column label="缺勤（次）" v-if="showUnAttendance">
                  <template slot-scope="scope1">
                    {{ scope1.row.workDays - scope1.row.checkDays - scope1.row.leaveDays }}
                  </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    deleteFixed(row) {
      this.$confirm("确定报废该资产吗？将会删除该资产", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        axios.post('/fixedasset/deleteById', { id: row.id }).then(res => {
          this.$message({ message: "资产已报废", type: "success" });
          this.getFixedList();
        })
      })
    },
    getCheckList() {
      axios.post('/check/getCheckList', {
        ...
        this.showUnAttendance = !this.isNowMonth(this.checkMonth)
      })
    },
    payOff(row) {
    ...
    isNowMonth(date) {
      ...
      // 给定的时间字符串
      var givenDateTimeString = date + '';
      var givenDate = new Date(givenDateTimeString);
      var givenYear = givenDate.getFullYear();
      var givenMonth = givenDate.getMonth() + 1; // 月份从0开始，需要加1
